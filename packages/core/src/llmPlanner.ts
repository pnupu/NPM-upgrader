...
export async function planWithLlm(diags: Diagnostic[], files: string[], cfg: LlmPlannerConfig): Promise<Plan> {
  ...
  const user = JSON.stringify({
    ...
    target: ctx.target ? { code: ctx.target.code, file: ctx.target.file, message: ctx.target.message } : undefined,
    targetSnippet: ctx.targetSnippet,
    focusText: ctx.focusText,
    importsHead: ctx.importsHead,
